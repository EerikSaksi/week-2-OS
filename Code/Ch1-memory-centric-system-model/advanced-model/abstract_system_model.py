...
# Initialise the intterupt vector table
    ...
    systemState = np.concatenate(
        (ramState, timerState, kbdState, nicState, ssdState, gpuState))
    ...
    for ncycles in range(0, MAX_NCYCLES):
        ...
        if PERIPHERALS:
            ...
            if (timerIrq):
                print('time_slice(systemState, registers)')
                time_slice(systemState, registers)
...
#            print( systemState[CODE:CODE+20] )
        (systemState, irqs, registers) = processorAction(
            systemState, irqs, registers, cfg)
